...
3. Migrate one component at a time. Parallelize across worktrees, one owner per component (the [Separate Before Serializing Shared State](../references/principles/separate-before-serializing-shared-state.md) principle). Shared primitives migrate first as a blocking phase.
