...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a `pstack:poteto-agent` subagent (your configured code-delegate profile). Review the diff. Capture a post-fix trace.
   Apply the [Sequence Verifiable Units](../references/principles/sequence-verifiable-units.md) principle, verifying each attempt before trying the next.
