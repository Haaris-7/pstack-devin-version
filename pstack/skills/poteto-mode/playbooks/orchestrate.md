...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Local agents are spawned, resumed, and drained only through `run_subagent`; cloud units are separate Devin sessions launched via `/handoff` or the Devin API. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nested spawns require `max-nesting` on the spawning profile; `pstack:poteto-agent` sets 2, cap programs at depth 3 regardless. Cloud placement is a separate Devin session, not a spawn flag). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Always a cloud Devin session (`/handoff`, or the Devin API for a standing program) unless the task needs this machine: `control-ui` or `control-cli` runtime verification (from `cursor-team-kit`). Reading the local session store (`sessions.db`). Simulators and local IDE state. Auth that exists only here. Cloud sessions cannot read the local session store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers. One writer per worktree or branch ([separate-before-serializing-shared-state](../references/principles/separate-before-serializing-shared-state.md)). Run a unit's verifier on a different model family from its worker.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the cloud session's status in the Devin web app (app.devin.ai). Session-store mtime is not liveness.
