...
- **Treating `auto` as a model slug.** `auto` and `inherit-parent` mean "spawn that role with `subagent_general` instead of a custom profile, so it inherits the parent session's model." [Setup](./01-setup.md) covers the roles.
